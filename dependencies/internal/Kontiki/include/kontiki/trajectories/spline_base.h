...
struct ControlPointInfo {
  ...
  virtual ceres::LocalParameterization* manifold() const {
...
class SplineView : public TrajectoryView<T, MetaType> {
 ...
 protected:
  ...
  virtual ceres::LocalParameterization* ControlPointManifold() {
    return nullptr;
  }
};
  ...
  struct SegmentEntity : public TrajectoryEntity<SegmentView,
    ...
    void AppendKnot(const ControlPointType& cp) {
      ...
      auto i = this->pstore_->AddParameter(this->control_point_info_.size,
                                           this->control_point_info_.manifold());
...
class SplineEntity : public TrajectoryEntity<SplineFactory<SegmentViewTemplate>::template View,
  ...
  SplineEntity(double dt, double t0, ceres::LocalParameterization* control_point_manifold) :
      segment_entity_(std::make_shared<SegmentType>(dt, t0)),
      control_point_manifold_(control_point_manifold) {
 ...
 public:
  ...
  SplineEntity(double dt, double t0) :
      SplineEntity(dt, t0, this->ControlPointManifold()) { };
  ...
  SplineEntity(const SplineEntity &rhs) :
    ...
    control_point_manifold_(this->ControlPointManifold())
  {
    this->segments.push_back(segment_entity_);
  }
  ...
  void AddToProblem(ceres::Problem &problem,
    ...
    for (auto tt : times) {
      ...
      for (int i=i1; i < (i2 + 4); ++i) {
        ...
        problem.AddParameterBlock(pi.data, pi.size, pi.manifold);
 ...
 protected:
  ...
  std::unique_ptr<ceres::LocalParameterization> control_point_manifold_;
};
