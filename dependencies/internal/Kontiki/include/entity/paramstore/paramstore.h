...
struct ParameterInfo {
  ParameterInfo(T* data, size_t size, ceres::LocalParameterization* manifold) :
    data(data), size(size), manifold(manifold) { };
...
struct ParameterStore {
  ...
  virtual size_t AddParameter(size_t ndims, ceres::LocalParameterization* manifold=nullptr) = 0;
